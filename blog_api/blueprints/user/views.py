...
from blog_api.blueprints.user.exceptions import (
    UserAlreadyExistError,
    UserDoesnotExistError,
    IncorrectPasswordError,
)
from blog_api.blueprints.user.models import User
from blog_api.blueprints.user.schema import (
    UserRegisterSchema,
    UserLoginSchema,
    UserResponseSchema,
)
...
def register():
    ...
    if existing_user:
        raise UserAlreadyExistError(
            "That email is taken. Try another.", status_code=409
        )
...
def login():
    ...
    password_authenticated = check_password(
        existing_user.password, user_credentials["password"]
    )
