...
class TestComment:
    def test_get_all_comments(self, client, test_comment):
        ...
        response = client.get(url_for("comment.get_all_comments", post_id=test_comment["post_id"]))
    ...
    def test_get_single_comment(self, client, test_comment):
        expected_status_code = 200
        response = client.get(url_for("comment.get_comment_by_id", comment_id=test_comment["id"]))
        response_data = response.get_json()
    ...
    def test_update_comment(self, client, test_comment, create_token):
        ...
        expected_status_code = 200
        response = client.put(url_for("comment.update_comment", comment_id=comment_id), json=payload,
                              headers={"Authorization": auth_token})
        response_data = response.get_json()
    ...
    def test_delete_comment(self, client, test_comment, create_token):
        ...
        response = client.delete(url_for("comment.delete_comment", comment_id=comment_id),
                                 headers={"Authorization": auth_token})
        response_data = response.get_json()
